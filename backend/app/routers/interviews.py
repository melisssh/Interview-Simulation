...
def create_interview(
    ...
    if not ai_list:
        logger.warning("Ollama soru üretimi boş döndü; fallback sorular kullanılacak.")
    ...
    try:
        ...
        if transcript_text:
            ...
            db.add(models.Feedback(
                interview_id=interview_id,
                scores_json=json.dumps(dict(analysis["scores"])),
                summary=analysis.get("summary", "")[:1000],
                strengths=analysis.get("strengths", "")[:2000],
                improvements=analysis.get("improvements", "")[:2000],
            ))
            db.commit()
